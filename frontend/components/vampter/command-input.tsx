...
export function CommandInput({ onExecute }: CommandInputProps) {
  ...
  const handleSelect = (id: string, name: string) => {
    setTarget(name)  // Use name for user-friendly display AND backend lookup
    setOpen(false)
  }
        ...
        <div className="flex flex-col gap-5 p-5 sm:p-6">
          ...
          <div className="flex flex-col gap-3">
            ...
            <div className="relative">
              ...
              />
              {open && (filtered.length > 0 || searchLoading || target) && (
                    ...
                    filtered.map((option) => (
                      <li key={option.id}>
                        <button
                          type="button"
                          onMouseDown={() => {
                            // Send the exact string key (name) as the company identifier
